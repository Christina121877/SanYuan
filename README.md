...
### v2.1 — FUI 赛博风格 + Bug 修复

- 🖥️ **FUI 风格重设计** — 深空底色 + 全息绿主色调 + 扫描线 + 信号条
- 💜 **蓝紫科技光粒子** — 150粒子 + 发光效果 + 十字星芒 + 粒子连线
- 🌐 **背景光晕 + 科技网格** — 三团浮动光晕 + 60px 半透明网格
- 🐛 **Bug 1 修复** — JS 筛选器 `s===A` 改为 `s==='A'`（引用未定义变量）
- 🐛 **Bug 2 修复** — 移除 `{total}` 二次重复替换
- 🧹 **清理** — 移除脆弱的 `.replace()` JS Bug 补丁
- 📏 **间距优化** — 加大 padding，解决界面拥挤问题
- 📖 **README 更新** — 移除旧版三省六部引用，更新功能描述
